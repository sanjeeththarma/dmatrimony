...
<div class="row">
    ...
    <?php
                ...
                $timg2;
                $g = $_SESSION["gender"];
                $sql = "SELECT * FROM profile WHERE id='$i' AND gender != '$g'";
                $result = mysqli_query($conn, $sql);
                if (mysqli_num_rows($result) > 0) {
                  echo '<script>it = "true"; </script>';
                    ...
                    while($row = mysqli_fetch_assoc($result)) {
                      ...
                      <script>
                        ...
                        $toname = '<b>To: </b>' . $row["fname"] . ' ' . $row["lname"];
                        echo '<br><br>';
                        $txt1;
                        $txt2;
                        $cnt=0;
    ...
    <div class="col-9">
      ...
      <p><img src="heart.png" style="width:50px; height:50px; position:relative; top:20px; left:50px;"><button class="button5" onclick="givelike('<?php echo $f; ?>','<?php echo $i; ?>')">Give Like</button><button class="button5" onclick="shwmsgbox1()">Send Message</button><button class="button5" onclick="shortlist('<?php echo $f; ?>','<?php echo $i; ?>')">Shortlist Profile</button></p>
    </div>
...
<div id="msgbox1" class="msgbg1">
<div class="close" style="top:-55px;" onclick="hideerr2()">&times;</div>
    <center>
      <?php echo '<font size="5" color="white">' . $toname . '</font>&nbsp;&nbsp;<img src="' . $simg[0] . '" style="width:60px; height:60px; border-radius:100%; position:relative; top:12px;"><br><br>'; ?>
    <textarea id="usrmsg" style="height:150px; max-width:500px; width:100%; border: 5px solid #618585; border-radius:5px;"></textarea>
    <br>
    <button class="button5" onclick="sendusrmsg('<?php echo $f; ?>','<?php echo $i; ?>')">Send</button>
    </center>
</div>
...
function viewed(){
  ...
  var t = '<?php echo $tv; ?>';
    var formData = new FormData();
        ...
        function sendusrmsg(f,t){
          if(it=="true"){
          if(document.getElementById('usrmsg').value != ""){
              var m1 = document.getElementById('usrmsg').value;
               var formData = new FormData();
                // Add the file to the AJAX request
                formData.append('from1',f);
                formData.append('to1', t);
                formData.append('msg1', m1);
                // Set up the request
                var xhr = new XMLHttpRequest();
                ...
                xhr.onload = function () {
                if (xhr.status == 200) {
                  var msg2 ='<br><center><input type="button" value="OK" name="ok" class="transbutton2" onclick="javascript:hideerr2()"></center>';
    ...
    document.getElementById('errmsg').innerHTML = '<center><font size="4">Empty message cannot be send !</font></center>' + msg2;
    document.getElementById('errmsg').style.display="block";
    centerDiv("errmsg");
            }
        }
